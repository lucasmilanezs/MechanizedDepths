...
ClientEvents.lang("en_us", function(event) {
    var materials = global._ticMaterials
    var outcomes  = global._ticOutcomes
    if (!materials || !outcomes) return

    for (var mi = 0; mi < materials.length; mi++) {
        var mat = materials[mi]
        var displayName = mat.name
            .split("_")
            .map(s => s.charAt(0).toUpperCase() + s.slice(1))
            .join(" ")

        event.add("tetra.material." + mat.name, displayName)
        event.add("tetra.material." + mat.name + ".prefix", displayName)
        event.add("tetra.material.feature." + mat.name, displayName)
    }

    var seen = {}
    var schematicKeys = Object.keys(outcomes)

    for (var si = 0; si < schematicKeys.length; si++) {
        var list = outcomes[schematicKeys[si]]

        for (var oi = 0; oi < list.length; oi++) {
            var outcome = list[oi]
            var mv = outcome.moduleVariant
            if (!mv || seen[mv]) continue
            seen[mv] = true
            ...
            var modName = _MV_NAMES[suffix] || suffix
            event.add("tetra.variant." + mv, mat + " " + modName)
        }
    }
})